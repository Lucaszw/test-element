...
class ModelObjectsController < ApplicationController
  ...
  def model_tile
     @model = UserModel.where("isPrivate = ? AND user_id = ? OR isPrivate = ? ", true, session[:id],false).find(:first, :order => "created_at DESC", :offset => params[:k])
     @k     = params[:k]
  ...
  def create
    ...
    @user = User.find(session[:id])
    @currentModel = UserModel.new(:isPrivate => params[:currentModel][:isPrivate])
